...
# -*- coding: utf-8 -*-
"""
旋转扩张子弹圈 — SkillEditor 通用技能模板生成器（v0.7）

== v0.7 净改动（2026-05-09，接力 a5049fcb670a71243，用户彻底否定 v0.6 散射退化路线）==
v0.5 已在 Unity 实测过，是对的方向。v0.6 错误地把"REPEAT+245 不是常规模式"当成
"项目不支持模板自驱 OnTick" — 用户实测 v0.5 能跑，反之 v0.6 散射版直接退化掉
OnTick 机制。v0.7 = v0.5 完整保留 + 新增 1 个 BulletConfig EXT_PARAM 槽位让
调用方可选地替换子弹外观/物理。

v0.7 仅两处改动：
  1. TemplateParams 9 → 10：新增 [10] [子弹] 子弹ID(BulletConfig)，默认 32300150
     （指向模板内置 BulletConfig，让用户不填时仍走 v0.5 全套 OnTick 路径）
  2. CREATE_BULLET.Params[0] 由硬编码 32300150 改为读 EXT_PARAM[10]

调用方使用语义：
  - 不填 / 填 32300150 → 走内置 BulletConfig，OnTick 自动跑（与 v0.5 一致）
  - 填别的 BulletConfig ID → 子弹外观/物理换成你的 BulletConfig；
    OnTick 是否生效取决于该 BulletConfig.AfterBornSkillEffectExecuteInfo
    是否也接到 32300101（不接则只是普通直线子弹）

== v0.5 净改动（保留，作为 v0.7 基线）==
...
== TemplateParams 10 项（v0.7，[10] 为 v0.7 新增）==
...
[10] bulletConfigID       BulletConfig 32300150 ✓ CREATE_BULLET.Params[0] 的子弹 ID（v0.7 新增）

注：v0.4 的 [1] bulletID 删除→v0.5 BulletConfig 内置→v0.7 重新暴露但默认指向内置 32300150
    （让调用方可选替换子弹外观/物理；不填仍走全套 OnTick 路径）。
...
== 拓扑 diff（v0.5 → v0.7）==
节点：完全相同（v0.7 不增不减节点，仅 TemplateParams 数组多 1 项 + Params[0] ParamType 改 4）
边：  完全相同
TemplateParams：v0.5 9 项 → v0.7 10 项（新增 [10] bulletConfigID）
...
- 【模板】扇形分层弹幕：BulletConfig EXT_PARAM 声明格式参考（RefTypeName/RefTableFullName/isConfigId）
"""
...
# 模板参数索引 (1-based, PT=4 EXTRA_PARAM) — v0.7 在 v0.5 9 项基础上 +1 = 10 项
EXT_N           = 1
EXT_R0          = 2
EXT_PHI         = 3
EXT_VR          = 4
...
EXT_MIN_R       = 8
EXT_FOLLOW      = 9
EXT_BULLET_CFG  = 10  # v0.7 新增：CREATE_BULLET.Params[0] 引用此槽位，默认 32300150
...
TPARAMS = [
    # [1] bulletCount N
    {"DefaultValueDesc": "子弹数量N。单帧同时生成的子弹个数（spawn 循环 RepeatExecute 次数）。\n"
                         "⚠️ 安全范围：N ≤ 50。超过 ~100 会触发 C++ 引擎 RepeatExecute 单帧次数上限保护"
                         "（报错 \"Execute Count Too Big\"，详见 PostMortem #026）。",
     "Name": "[阵型] 子弹数N", "RefTypeName": "",
    ...
    {"DefaultValueDesc": "0=圆心固定在 cast 位置(古早风格)；1=每帧跟随 caster(现代 MOBA 标配)。默认 1。",
     ...
     "RefTableFullName": "", "RefTableManagerName": "",
     "RefPortTypeName": "", "RefPortTypeFullName": "", "IsFunctionReturn": False},
    # [10] bulletConfigID — v0.7 新增（参考扇形分层弹幕「子弹ID」声明格式）
    {"DefaultValueDesc": "调用方传 BulletConfig.ID 决定子弹外观/物理；不填或填 32300150 → "
                         "走模板内置子弹（OnTick 自动跑）；填其他 ID → 走外部子弹（外部 "
                         "BulletConfig 需自行手配 AfterBornSkillEffectExecuteInfo=32300101 "
                         "才能让 OnTick 在该子弹上生效）。默认 32300150 = 模板内置 BulletConfig。",
     "Name": "[子弹] 子弹ID(BulletConfig)",
     "RefTypeName": "BulletConfig", "RefPortTypeNames": "",
     "DefalutParam": {},
     "DefalutParamJson": '{"Value":32300150,"ParamType":0,"Factor":0}',
...
def build():
    ...
    # ===== v0.7 关键改造：CREATE_BULLET.Params[0] 改读 EXT_PARAM[10] =====
    # v0.5：硬编码 P(BULLET_CONFIG_ID) 死值
    # v0.7：P(EXT_BULLET_CFG, PT_EXTRA_PARAM) → 默认值 32300150（指向模板内置 BulletConfig）
    #       调用方在面板填别的 BulletConfig.ID 即可替换子弹外观/物理
    bullet_eid = b.alloc_eid()
    bullet_payload = {
        "ID": bullet_eid, "SkillEffectType": ET["BULLET"],
        "Params": [
            P(EXT_BULLET_CFG, PT_EXTRA_PARAM),      # 0  v0.7：读 EXT_PARAM[10]，默认 32300150
    ...
    b.add_node("TSET_CREATE_BULLET", bullet_eid, bullet_payload,
               desc="创建第 i 颗子弹（v0.7：BulletConfig 由 EXT_PARAM[10] 决定，默认 32300150）",
    ...
    b.add_node(
        ...
        desc="旋转扩张子弹圈模板根 v0.7（10 项 EXT_PARAM = v0.5 9 项算法 + [10] BulletConfig）",
        effect_type=ET["ORDER"])
...
def make_sticky_notes() -> list:
    """v0.7 sticky note — v0.5 基础 + 子弹ID EXT_PARAM 说明"""
    overview = (
        "[作用]\n"
        "通用旋转扩张子弹圈模板，参考木宗门 30212010 叶散风行的 OnTick 机制。\n\n"
        "[流程]\n"
        "1. 拖入模板 → 默认走内置 BulletConfig (32300150)，OnTick 自动跑\n"
        "2. 创建 N 颗子弹按圆周均匀分布在 R0 半径\n"
        "3. OnTick 每帧（通过 BulletConfig.AfterBorn 挂的 32300101 工具子图）：\n"
        "   θ_i += ω, R += vR + aR/30, 重算每颗位置\n"
        "4. 子弹长边沿切线（φ=0）或偏移 φ 角朝向\n"
        "5. R > maxR / R < minR / 子弹时长到 → 自动销毁\n\n"
        "[特殊条件]\n"
        "- 默认子弹ID = 32300150（内置）→ OnTick 自动跑，新手拖入即用\n"
        "- 改子弹ID = 别的 BulletConfig ID → 子弹外观换成你的，但 OnTick 是否生效\n"
        "  取决于你的 BulletConfig 有没有挂 AfterBorn=32300101\n"
        ...
        "  1=`==` 2=`!=` 3=`>` 4=`>=` 5=`<` 6=`<=`\n\n"
        "[参数] 10 项 EXT_PARAM（v0.7 在 v0.5 9 项基础上新增 [子弹] 子弹ID）\n"
        "[子弹] 子弹ID(BulletConfig)：默认 32300150 走内置；填别的 ID 替换外观\n"
        "[阵型] 子弹数N              默认 8\n"
        ...
        "子弹外观/生命/特效字段（不走 EXT_PARAM）：\n"
        "  Model：改内置 ModelConfigNode (默认 320149 普通飞叶) — 仅默认子弹生效\n"
        "  LastTime / Speed / DieSkillEffectExecuteInfo：直接编辑内置 BulletConfigNode\n"
        "  ModelBaseScalePercent / Hp 等：同上\n\n"
        "[联动]\n"
        "- 默认走内置子弹 → 拖入即用\n"
        "- 替换 BulletConfig 时调用方需自行确保 AfterBorn 接好（不再是模板义务）\n"
        "- 调用方在自己的 SkillConfig.SkillDamageTagsList 加伤害 tag 即可给子弹挂伤害\n"
        "- 调用方使用 TSET_RUN_SKILL_EFFECT_TEMPLATE (SET=118) 引用本模板根 32300001\n\n"
        "[v0.7 净改动 vs v0.5]\n"
        "v0.5 实测过是对的方向，v0.6 散射版退化错误。v0.7 = v0.5 完整保留 +\n"
        "新增 1 个 EXT_PARAM 让调用方可选替换子弹：\n"
        "1) 新增 [10] [子弹] 子弹ID(BulletConfig) — RefType=BulletConfig，默认 32300150\n"
        "2) CREATE_BULLET.Params[0] 由硬编码 32300150 改为读 EXT_PARAM[10] (PT=4)\n"
        "其余节点/边/字段端口边/9 项算法 EXT_PARAM 完全保留。\n\n"
        "[参考真实样本]\n"
    ...
    return [{
        ...
        "title": "[概览] 旋转扩张子弹圈 通用模板 v0.7 — v0.5 + BulletConfig EXT_PARAM",
        "content": overview,
    }]
...
def main():
    ...
    print(f"  TemplateParams 数量: {len(TPARAMS)} (v0.5: 9; v0.7 新增 [10] BulletConfig)")
