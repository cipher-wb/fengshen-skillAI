...
export async function doctorCommand(targetPath, options) {
  ...
  // [7] Python 工具依赖 (pyyaml, jsonschema, openpyxl)
  const pyExec = config?.python_executable || 'python';
  try {
    await exec(`${pyExec} -c "import yaml, jsonschema, openpyxl"`);
    checks.push({ id: 7, status: 'pass', msg: 'Python 依赖齐全 (pyyaml, jsonschema, openpyxl)' });
  } catch (err) {
    checks.push({
      id: 7,
      status: 'warn',
      msg: `Python 依赖缺失或 ${pyExec} 不可用 (运行 pip install -r doc/SkillAI/tools/requirements.txt)`
    });
  }
