...
export async function initCommand(targetPath, options) {
  ...
  ${chalk.dim('Node 18+ / Python 3.10+ (含 pip) / Git / Claude Code')}
  ...
  ${chalk.dim('# 4. 在 Claude Code 内开始使用：')}
  $ claude
  > 配一个木宗门的直线子弹技能 (示例)

  ${chalk.dim('# 5. 健康检查 (随时跑 / 自动检测 Python+各项配置)：')}
  $ npx fengshen-skillai doctor

  ${chalk.dim('# 6. 想看完整学习痕迹？(可选 / ~3.5MB tarball)：')}
