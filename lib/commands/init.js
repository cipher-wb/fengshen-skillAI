...
export async function initCommand(targetPath, options) {
  ...
  ${chalk.dim('# 1. 安装 Python 工具依赖：')}
  $ cd ${projectRoot}
  $ pip install -r doc/SkillAI/tools/requirements.txt

  ${chalk.dim('# 2. 配置个人 user_config.json：')}
  $ cp doc/SkillAI/tools/user_config.json.example doc/SkillAI/tools/user_config.json
  # 编辑 user_config.json → 设置你的 IPv4 末段等

  ${chalk.dim('# 3. 在 Claude Code 内开始使用：')}
  $ claude
  > 配一个木宗门的直线子弹技能 (示例)

  ${chalk.dim('# 4. 健康检查 (可随时跑)：')}
  $ npx fengshen-skillai doctor

  ${chalk.dim('# 5. 想看完整学习痕迹？(可选 / ~44MB)：')}
